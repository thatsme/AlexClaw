...
| Telegram bot | Owner chat ID + owner user ID; blank answers nothing |
| Discord bot | Owner channel ID + owner user ID; blank answers nothing |
| OpenBao | Internal network, TLS, AppRole bound to AlexClaw's address, least-privilege policy |
| Web automator sidecar | Bearer token, own network, no published API port |
...
- **No hardcoded secrets** — bootstrap values come from the environment; every credential is kept in OpenBao
