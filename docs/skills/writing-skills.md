...
| `secret_config_keys/0` | `[]` | Config keys holding credentials, kept in OpenBao as secrets the step owns, bound to the host of the step's `url` (e.g. `["api_key"]`); the step's config holds a reference, and the skill receives a placeholder, `{{secret:NAME}}`, never the value. A placeholder in a request made through `SkillAPI.http_request` (URL, header or body) is filled as the request is sent, for the host it goes to and only if that is the host the secret is bound to. A key whose value is a map of headers keeps only its credential headers secret. Required for every key in `config_scaffold/0` or `config_presets/0` with a name one of whose underscore-separated parts is `token`, `key`, `apikey`, `password`, `secret`, `credential`, `auth`, `authorization` or `headers` (so `api_key` and `auth_header` count, `keyword_count` does not): a skill that leaves one out is refused at load |
