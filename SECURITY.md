...
- **Privileged steps run only in a run the scheduler starts, or one the admin UI starts with a code** — a workflow step of `shell`, `coder`, `db_backup` or `web_automation` in a run started from MCP, a webhook, a chat (even with a code) or another node fails at that step, naming it, and the skill does not run. The control plane decides from the entry point; a caller cannot claim it. The admin UI asks for a code to run a workflow that has such a step
