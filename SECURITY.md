...
- **Privileged steps run only in a run the scheduler starts, or one the admin UI starts with a code** — a run of a workflow with a `shell`, `coder`, `db_backup` or `web_automation` step, started from MCP, a webhook, a chat (even with a code) or another node, is refused before it starts, naming the step: no step of it runs. The control plane decides from the entry point; a caller cannot claim it. The admin UI asks for a code to run a workflow that has such a step, and does not offer the chat that approval
