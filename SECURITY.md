...
**Sensitive config stored as plaintext in PostgreSQL.**
API keys and tokens are stored unencrypted in the database. Access to the
database directly (e.g. via a stolen backup) exposes these values.
Mitigation: restrict PostgreSQL access at the network level; never expose
port 5432 publicly.
...
- Set a strong random `SECRET_KEY_BASE` (`mix phx.gen.secret`)
