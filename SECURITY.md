...
- **2FA on every manual load** — load, unload and reload require TOTP verification via Telegram/Discord, from both the Admin UI and the `/skill` commands, and are refused when 2FA is not configured. **This does not cover skills generated by Coder/Forge** — see below
...
> **These load generated code without TOTP verification.** `/coder` and the
> Forge admin page generate a skill with a local LLM and load it into the
> running VM immediately. The only check is that the calling skill holds the
> `:skill_manage` permission — there is no second factor, and no human approves
> the code before it runs. A loaded skill has full VM privileges, so this is
> equivalent to letting the model deploy code.
>
> Every other route to loading a skill is 2FA-gated. This one is not.
>
> **If that is not acceptable for your deployment, remove them.** There is no
> runtime toggle: drop `"coder" => AlexClaw.Skills.Coder` from the core skill
> map in `lib/alex_claw/workflows/skill_registry.ex` and the
> `live("/forge", AdminLive.Forge)` route from `lib/alex_claw_web/router.ex`,
> then rebuild. Generation is only as trustworthy as the model producing the
> code and the prompt reaching it — note that a `/coder` goal can arrive as a
> gateway message.

The `/coder` command uses a local LLM to generate dynamic skills from natural
language descriptions. Generated code passes through the same load-time
validation as manually-loaded skills (one module per file, namespace, module
body restrictions, behaviour, permission checks). Additional safety measures:
