defmodule AlexClaw.Memory do
  ...
  defp store_with_chunks(attrs) do
    ...
    case %Entry{} |> Entry.changeset(attrs) |> Repo.insert() do
      {:ok, parent} ->
        attrs.content
        |> Chunker.chunk()
        |> Enum.each(&store_chunk(&1, attrs, parent))
  ...
  defp store_chunk(%{text: text, index: idx}, attrs, parent) do
    chunk_attrs = %{
      kind: attrs.kind,
      content: text,
      source: attrs.source,
  ...
  def reembed_all(opts \\ []) do
    ...
    start_reembed(entries, count, opts, count > 0 and embedding_possible?())

    {:ok, count}
  end

  defp start_reembed(_entries, _count, _opts, false), do: :ok

  defp start_reembed(entries, count, opts, true) do
    caller = self()
  ...
  defp current_embedding_model do
    AlexClaw.LLM.Embedding.model() || @default_embedding_model
  end
