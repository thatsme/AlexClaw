#!/bin/sh
# The one way to run the whole python suite: a gate (.claude/rules/20-tests.md) —
...
if [ "$(uname -s)" = Darwin ] && command -v caffeinate >/dev/null 2>&1; then
  GATE=1 exec caffeinate -dimsu ./scripts/test-python.sh
fi
GATE=1 exec ./scripts/test-python.sh
